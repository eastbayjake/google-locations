...
GoogleLocations.prototype.textsearch = function(options, cb) {
  ...
  this._makeRequest(this._generateUrl(options, 'place', 'textsearch'), cb);
};
...
GoogleLocations.prototype.searchByAddress = function(config, cb) {
  ...
  _self.geocodeAddress(options.geocode, function(err, result){
    ...
    _self.search(query, function(err, result){
      ...
      var details = [];
      var errors = [];
      var j = (options.search.maxResults > result.results.length) ? result.results.length : options.search.maxResults;
      for (var i = 0; i < j; i++) {
        try {
...
GoogleLocations.prototype.searchByPhone = function(options, cb) {
  ...
  _self.textsearch({query: 'phone number ' + options.phone}, function(err, result){
    ...
    var details = [];
    var errors = [];
    var j = (options.maxResults && options.maxResults < result.results.length) ? options.maxResults : result.results.length;
      for (var i = 0; i < j; i++) {
        try {
          var placeid = result.results[i].place_id;
          _self.details({placeid: placeid}, function(err, result){
